  ...
  <div class="min-h-screen bg-gradient-to-b from-surface-950 via-surface-900 to-surface-950 text-white">
    <div class="mx-auto w-full max-w-6xl px-4 sm:px-6 py-16 space-y-10">
      ...
      <header class="space-y-4 text-center">
        ...
        <div class="space-y-2">
          <p class="text-xs uppercase tracking-[0.3em] text-white/40">Favoritos compartidos</p>
          <h1 class="text-3xl font-semibold sm:text-4xl">Lista de favoritos</h1>
          <p class="text-sm text-white/60">Películas y series guardadas por este usuario en Recomiéndame.</p>
        </div>
      </header>
      ...
      <!-- CTA: abrir app o descargar -->
      <div class="rounded-3xl border border-white/10 bg-white/5 p-6 sm:p-8 space-y-5 text-center">
        <h2 class="text-xl font-semibold">¿Quieres tu propia lista de favoritos?</h2>
        <p class="text-sm text-white/60 max-w-md mx-auto">
          Descarga Recomiéndame, guarda tus películas y series favoritas y recibe recomendaciones personalizadas con IA.
        </p>
        ...
        <!-- Links de descarga (se muestran si la app no abre) -->
        <transition name="fade">
...
const openApp = () => {
  ...
  // Intentar abrir la app; si no responde en 1.5s, mostrar links de tiendas
  const timer = setTimeout(() => {
    showStoreLinks.value = true;
  }, 1500);

  // Si la página pierde foco, la app se abrió correctamente
  const onBlur = () => {
    clearTimeout(timer);
...
useHead({
  title: "Favoritos compartidos | Recomiéndame",
  meta: [
    { name: "description", content: "Mira los favoritos de este usuario en Recomiéndame." },
  ],
});
</script>
