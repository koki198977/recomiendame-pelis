    ...
    <div class="max-w-md w-full space-y-8">
      <div class="bg-white rounded-3xl shadow-soft p-8">
        ...
        <div class="text-center mb-8">
          <div
            class="w-16 h-16 bg-primary-500 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-medium"
          >
...
useHead({
  ...
  meta: [
    {
      name: "description",
      content: "Establece tu nueva contraseña para Recomiéndame Coach.",
    },
  ],
});
...
if (!token) {
  throw createError({
    statusCode: 404,
    statusMessage: "Token de restablecimiento no válido",
  });
}
...
const handleSubmit = async () => {
  if (!isFormValid.value) {
    error.value =
      "Por favor verifica que las contraseñas coincidan y tengan al menos 6 caracteres";
    return;
  }
  ...
  try {
    ...
    if (err.status === 400) {
      ...
      error.value =
        "Hubo un error al actualizar tu contraseña. Por favor intenta de nuevo.";
    }
  } finally {
    loading.value = false;
  }
