              ...
              <div class="space-y-4">
                ...
                <div class="flex flex-wrap gap-2">
                  ...
                  <button
                    type="button"
                    class="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-white/80 transition hover:bg-white/10"
                    @click="selectedItem ? openShareFromDetails(selectedItem) : null"
                  >
                    🔗 Compartir
                  </button>
                  <button
                  ...
                  >
                    <span v-if="deletingId !== String(selectedItem?.tmdbId)">🗑️ Eliminar de wishlist</span>
...
const ratingTarget = ref<WishlistItem | null>(null);
...
const openDetails = (item: WishlistItem) => {
...
const openShareFromDetails = (item: WishlistItem) => {
  // Aquí puedes implementar la funcionalidad de compartir
  console.log('Compartir:', item);
};

const handleRemoveFromDetails = async (item: WishlistItem) => {
...
const openRating = (item: WishlistItem | null) => {
  if (!item) return;
